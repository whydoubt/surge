...
{
public:
   CParameterTooltip(const VSTGUI::CRect& size) : CControl(size, 0, 0, 0)
   {
      label[0][0] = 0;
      label[1][0] = 0;
      visible = false;
   ...
   virtual void draw(VSTGUI::CDrawContext* dc)
   {
      if (visible)
      {
         // VSTGUI::COffscreenContext *dc =
         // VSTGUI::COffscreenContext::create(getFrame(),size.width(),size.height());
	 ...
	 VSTGUI::CRect tupper(trect), tlower(trect);
         ...
         // dc->drawString(label[1],tlower,false,label[0][0]?VSTGUI::kRightText:VSTGUI::kCenterText);
